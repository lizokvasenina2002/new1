...
% Определение функций
function paths = findPaths(field, robot, target_points, target_order, disp_flag, disp_flag2, start_tick, robot_index, axes_handles)
    ...
    while target_index <= length(target_order)
        ...
        % Очистка предыдущего пути и подготовка к следующему
...
for robot_index = 1:length(robots)
    ...
    all_paths{robot_index} = findPaths(field, robots{robot_index}, target_points, target_order, disp_flag, disp_flag2, start_tic, robot_index, axes_handles);
    ...
    % Проверка, достиг ли робот целевой точки
    for k = 1:size(paths{robot_index}, 1)
        if ismember([paths{robot_index}(k, 1), paths{robot_index}(k, 2)], target_points, 'rows')
            % Если достиг, очистить предыдущий путь
            clearPreviousPath(robot_index, field, target_points, axes_handles);
        end
    end
end

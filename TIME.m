    ...
    while target_index <= length(target_order)
        ...
        while ~isequal(current_position, robot)
            if visited(current_position(1), current_position(2)) ~= 1
                reverse_path = [reverse_path; current_position];% sum
        ...
        for i = 2:size(path, 1)
            current_time = current_time + 1; %field(path(i, 1), path(i, 2));
            path(i, 3) = current_time;
            sum = sum + 1;

            %добавить координаты path столько раз сколько раз будет
            %вычитаться 1 из field
...
for i = 1:length(robots)
    ...
    disp(paths{i});
%{
    % Проверка, выполнил ли робот все свои задания
    if length(all_target_orders{i}) == length(paths{i})
        % Назначить новое задание роботу
        prompt = sprintf('Введите новые целевые точки для робота %d (например, [2, 4, 5, 6, 7]): ', i);
        all_target_orders{i} = input(prompt);
        % Найти новый путь для робота
        all_paths{i} = findPaths(field, robots{i}, target_points, all_target_orders{i}, disp_flag, {});
        paths{i} = vertcat(all_paths{i}{:});
        disp(['Новые шаги ', num2str(i), ' робота:'])
        disp(paths{i});
    end
%}
end

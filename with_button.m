...
% Определение функции поиска пути
...
visualization_panel = uipanel(visualization_fig, 'Position', [0.05 0.1 0.9 0.9], 'Title', 'Визуализация путей');
...
button_panel = uipanel(visualization_fig, 'Position', [0.05 0.05 0.9 0.1], 'Title', 'Управление роботами');

% Создание кнопок для каждого робота
global robot_buttons;
robot_buttons = zeros(1, length(robots));
for i = 1:length(robots)
    robot_buttons(i) = uicontrol('Style', 'pushbutton', 'String', ['Завершение работы робота ', num2str(i)], ...
                                 'Position', [20 + (i-1)*250, 20, 180, 30], 'Parent', button_panel, ...
                                 'Callback', {@moveRobotToTarget, i});
end
...
% Функция для перемещения робота в целевую точку [6, 1]
function moveRobotToTarget(~, ~, robot_index)
    global field robots target_points disp_flag disp_flag2 paths all_paths colors visualization_fig axes_handles current_steps current_times start_time start_tic;
    ...
    if ~isempty(all_paths{robot_index})
        % Получаем текущий путь робота
        current_path = paths{robot_index};

        % Обновляем начальную позицию робота для следующего задания
        robots{robot_index} = current_path(end, 1:2);

        % Обновляем глобальное текущее время до конца текущего пути робота
        current_times(robot_index) = current_path(end, 3) + 1;
    end

    % Генерация нового пути для робота в целевую точку [6, 1]
    target_order = [7]; % Целевая точка [6, 1]
    start_tic = current_steps(robot_index);
    all_paths{robot_index} = findPaths(field, robots{robot_index}, target_points, target_order, disp_flag, disp_flag2, start_tic);
    paths{robot_index} = vertcat(all_paths{robot_index}{:});

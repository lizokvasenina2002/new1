...
%дали задание роботу 1, нашли путь, робот 2 и 3, проверка на пересечение, визуализация
while true
    paths = cell(length(robots), 1); % Для хранения путей всех роботов
    all_paths = cell(length(robots), 1); % Для хранения всех путей
    
    % Обработка каждого робота поочередно
    for i = 1:length(robots)
        % Ввод целевых точек для текущего робота
        prompt = sprintf('Введите целевые точки, которые должен посетить робот %d (например, [2, 4, 5, 6, 7]) или "exit" для завершения: ', i);
        ...
        disp(['Поиск пути для робота ', num2str(i)]);
        all_paths{i} = findPaths(field, robots{i}, target_points, target_orders, disp_flag);
        paths{i} = vertcat(all_paths{i}{:});
        
        % Обновить начальную позицию робота для следующего задания
        robots{i} = paths{i}(end, 1:2);
    end
...
%{
%с ошибкой robots{q} = paths{q}(end, 1:2); Index in position 1 is invalid. Array indices must be positive integers or logical values.
while true
    ...
    for i = 1:length(robots)
        ...
        all_target_orders{i} = str2num(input_str); %#ok<ST2NM>
    %end
        ...
        for q = 1:length(robots)
            % Найти путь для роботов
            all_paths{q} = findPaths(field, robots{q}, target_points, all_target_orders{q}, disp_flag);
            paths{q} = vertcat(all_paths{q}{:});
        
            % Обновить начальную позицию робота для следующего задания
            robots{q} = paths{q}(end, 1:2);
        end
    
        
        
        % Проверка на пересечение траекторий и добавление ожидания
        for w = 1:length(paths)
            for j = w+1:length(paths)
                for k = 1:size(paths{w}, 1)
                    for l = 1:size(paths{j}, 1)
                        % Проверка на пересечение координат Inf на одном и том же такте
                        if paths{w}(k, 3) == paths{j}(l, 3) || paths{w}(k, 3) == paths{j}(l, 3) + 1
                            inf_coords_w = updateObstaclesAroundRobot(field, paths{w}(k, 1:2));
                            inf_coords_j = updateObstaclesAroundRobot(field, paths{j}(l, 1:2));
        
                            % Проверка на пересечение координат Inf
                            if any(ismember(inf_coords_j, [paths{w}(k, 1), paths{w}(k, 2)], 'rows')) && any(ismember(inf_coords_w, inf_coords_j, 'rows')) && any(ismember(inf_coords_w, [paths{j}(l, 1), paths{j}(l, 2)], 'rows'))
                                % Добавляем ожидание для второго робота
                                paths{j} = [paths{j}(1:l-1, :); paths{j}(l, :) + [0, 0, 1]; paths{j}(l:end, :) + [0, 0, 1]];
                            end
                        end
        ...
        for e = 1:length(paths)
            for j = e+1:length(paths)
                for k = 1:size(paths{e}, 1)
                    for l = 1:size(paths{j}, 1)
                        % Проверка на посещение целевой точки
                        if ismember([paths{e}(k, 1), paths{e}(k, 2)], target_points, 'rows') && ismember([paths{j}(l, 1), paths{j}(l, 2)], target_points, 'rows')
                            % Проверка на пересечение времени
                            if paths{e}(k, 3) == paths{j}(l, 3)
                                % Добавляем ожидание для второго робота
                                paths{j} = [paths{j}(1:l-1, :); paths{j}(l, :) + [0, 0, 1]; paths{j}(l:end, :) + [0, 0, 1]];
                            end
                        end
        ...
        for r = 1:length(paths)
            paths{r}(:, 3) = 0:(size(paths{r}, 1)-1);
        end
        
        for t = 1:length(robots)
            disp(['Шаги ', num2str(t), ' робота:'])
            disp(paths{t});
        end
...
%{
%последовательный ввод заданий для роботов
while true
    % Собрать все задания для роботов
    all_target_orders = cell(length(robots), 1);
    for i = 1:length(robots)

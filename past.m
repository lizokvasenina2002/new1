...
function paths = findPaths(field, robot, target_points, target_order, disp_flag, all_robot_paths, robot_index)
    ...
    for i = 1:size(target_points, 1)-1
    ...
    while target_index <= length(target_order)
        ...
        for i = 2:size(path, 1)
            ...
            updateObstaclesAroundRobot(field, path(i, 1:2));
        end
...
function updateObstaclesAroundRobot(field, robot)
...
for i = 1:length(robots)
    ...
    all_paths{i} = findPaths(field, robots{i}, target_points, all_target_orders{i}, disp_flag, {}, i);
    ...
    for j = i+1:length(paths)
        for k = 1:size(paths{i}, 1)
            for l = 1:size(paths{j}, 1)
                if paths{i}(k, 1) == paths{j}(l, 1) && paths{i}(k, 2) == paths{j}(l, 2) && paths{i}(k, 3) == paths{j}(l, 3)
                    % Если траектории пересекаются в один и тот же такт, добавляем ожидание
                    paths{j} = [paths{j}(1:l-1, :); paths{j}(l, :) + [0, 0, 1]; paths{j}(l:end, :)];
                    disp(['Робот ', num2str(j), ' ждёт в позиции (', num2str(paths{j}(l, 1)), ',', num2str(paths{j}(l, 2)), ') в такт ', num2str(paths{j}(l, 3))]);
                end
            end
        end
    end
